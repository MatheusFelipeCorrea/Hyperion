...
- **Windsurf and Zed runtime rules** (`.windsurfrules`, `.rules`) alongside the existing Claude/Cursor/Copilot targets — same command-table sync via `hyperion:generate-rules`, same nested-adoption shims via `install-product-shims.mjs`.
- **"How to add a new backend" guide** in `scripts/cards-sync/README.md` — a real 10-step walkthrough of the shared pattern across all 4 existing backends (request helper, `CARD_ID` idempotency marker, dependency-injected linking function, reverse-sync shape, wiring points), replacing a "(roadmap)" stub.
...
- **`commands.yml` ↔ filesystem parity test** — generalizes the `package.json`↔`cli.mjs` `COMMANDS` check to the hand-maintained skill/agent/npm-shortcut registry: a typo'd skill name used to silently render a glob-pattern string in `CLAUDE.md`/`hyperion.mdc` forever with nothing failing.
