...
Todo item que este arquivo listava em Agora/Próximo/Mais pra frente foi implementado — `feat/agora-lane-items`, PR #80. Destaques: dry-run-by-default em todo cards-sync (`migrate-layout.mjs` alinhado), teste isolado de hierarquia pai-filho nos 4 backends, E2E cobrindo reverse-sync, `hyperion:project-fields-apply`, `hyperion:audit-diff`, servidor MCP próprio (doctor + project-verify, sem dependência de SDK), `hyperion:create` (scaffold local), `cards:history`, notificador Slack/Discord, `labels-reset.mjs` estendido (GitLab + correção real do Linear), license-checker, 4 skills novos de cobertura de SDLC, telemetria opt-in, concorrência limitada no sync, e 2 ADRs (split de `sync.mjs`/`lib.mjs` e GitHub App vs PAT — avaliados e adiados por decisão registrada, não esquecidos).

3 itens desta rodada foram deliberadamente **deixados de fora** — cada um já existia como [good first issue](https://github.com/MatheusFelipeCorrea/Hyperion/labels/good%20first%20issue) aberto pra comunidade antes deste plano começar, e implementá-los aqui fecharia a porta de entrada de um contribuidor: guia de "como adicionar um backend novo" (#75), suporte a Windsurf/Zed em `RUNTIME_TARGETS` (#79), teste de paridade `commands.yml`↔filesystem (#77).
